...
#' A column r2.vals.unique is also present. It is NA unless report.unique.r2 is TRUE, in which case it is the model R2 minus the R2 of the null model, that is, the variance explained beyond the terms supplied in null.terms. Where null.terms is empty the null model's formula is the intercept alone and the column equals r2.vals. Note that this drops every term of the test.fit's formula, a random effect written as s(site, bs = 're') included; that is what null.terms exists to put back. A random structure supplied through a separate argument rather than through the formula, as in uGamm(random = ~(1|site)), is not part of the formula and is retained by the null model either way.
#' Two qualifications. The column is a per-model quantity, not a variance partition among terms: with max.predictors greater than one it is the joint contribution of every predictor in that model, and it is a per-predictor contribution only at max.predictors = 1. And it is on whatever scale r2.type produced, so a candidate fitting worse than the null on the chosen measure gives a negative value.
